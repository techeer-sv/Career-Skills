...
이력서를 **가장 비판적인 면접관**의 눈으로 읽고, 라이브 면접에서 파고들리거나 감점될 지점만 골라낸다. `hiring-sim-resume-review`가 "ATS + 리크루터 합격선 스크리닝"이라면, 이 스킬은 **"어디서 깎이고 어디서 털리는가"를 포렌식으로 적발**하는 레드팀 렌즈다.
...
## 이 스킬 vs `hiring-sim-resume-review`

| 축 | resume-review (기존) | **resume-redteam (이 스킬)** |
|---|---|---|
| 산출물 | PASS/REJECT + ATS·리크루터 결과 | **심각도 태그된 감점 포인트 목록** |
| 목적 | 합격선 스크리닝 | 면접관이 파고들 약점 사냥 |
| 수치 | 성과 유무·밀도 판정 | **규모 추정·리서치로 "말이 되는가"까지** |
| 링크 | 학습 흔적 유무 체크 | **GitHub 코드·블로그를 직접 열어 주장과 대조** |
| 렌즈 | 루브릭 | **Cartesian doubt(방법적 회의)** |

보완재다. 합격선은 `-review`, 디테일 감점·모순 적발은 이 스킬. 포트폴리오도 있으면 `hiring-sim-portfolio-redteam`과 짝으로.
...
> 9번은 "틀린 것"이 아니라 "손해 보는 것". 감점과 **별개 축**으로, 각 밋밋한 항목을 `문제→해결(기술 판단)→정량 성과` 구조로 끌어올릴 지점을 제안한다(`hiring-sim-resume-review`의 [문제-해결-성과] 서사 기준과 연계). **실제 한 것만** 쓰도록 경고.
...
## 🎣 밋밋 → 시니어 서사 (틀리진 않았지만 손해 보는 것)
- **<항목>** — 빈 칸: <문제 맥락 / 기술 판단 근거 / 정량 성과 중 없는 것>
  - Before: ~~"<원문 그대로>"~~ → After: "<문제→판단→성과로 끌어올린 버전>" (실제 한 것만)
...
- **개선안은 실행 가능하게.** 감점만 던지지 말고 "이렇게 바꿔라"를 항상 붙인다.
